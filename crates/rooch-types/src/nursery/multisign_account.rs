...
pub fn generate_multisign_address(
    ...
    let multisig_script = create_multisig_script(threshold, &mut public_keys);
    ...
    let internal_key = bitcoin::XOnlyPublicKey::from(public_keys[0]);
...
fn create_multisig_script(
    threshold: usize,
    public_keys: &mut Vec<bitcoin::PublicKey>,
) -> ScriptBuf {
    ...
    for pubkey in public_keys {
        if builder.is_empty() {
            builder = builder.push_x_only_key(&XOnlyPublicKey::from(*pubkey));
...
mod tests {
    ...
    fn test_multisign_address_generate_with_fix_pubkey() {
        ...
        for (threshold, pubkeys, expected_address) in cases {
            let pubkeys = pubkeys
                .iter()
                .map(|pk| bitcoin::PublicKey::from_str(pk).unwrap())
                .collect();
...
"03786e2d94b8aaac17b2846ea908a245ab8b3c9df7ff34be8c75c27beba8e1f579"],
        ...
        for (threshold, pubkeys, expected_script) in cases {
            ...
            let script = create_multisig_script(threshold, &mut pubkeys.clone());
